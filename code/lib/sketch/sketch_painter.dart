...
class SketchPainter extends CustomPainter {
  ...
  final String? draggingObjectId;  // id of object being dragged (null if none)
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (points.isNotEmpty) {
      ...
      _drawRoom(canvas);
      if (isClosed) _drawRoomObjects(canvas);
      _drawPoints(canvas);
  ...
  void _drawRoomObjects(Canvas canvas) {
    ...
    for (final obj in roomObjects) {
      if (obj.wallIndex >= (isClosed ? points.length : points.length - 1)) continue;

      // Find centre point in screen coords
      final centre = objectCentreWorld(
        obj: obj,
        points: points,
        wallCount: isClosed ? points.length : points.length - 1,
      );
